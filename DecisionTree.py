    ...
    def load_file(self, file_name):
        with open(file_name, 'r') as f:
            for line in f:
                ...
                self.add_instance(self.dic[data[0]], [self.dic[x] for x in data[1:]])
        self.make_attr_set()
        return self
...
def compute_entropy(data: Instances):
    ...
    x = Counter(data.label).most_common()[0][1]
    if x == data.num_instances:
        return 0
    a = x / data.num_instances
    b = (data.num_instances - x) / data.num_instances
    total_entropy = -a * np.log(a) - b * np.log(b)
    return total_entropy


def compute_info_gain(data: Instances, att_idx):
    info_gain = 0.0
    ########## Please Fill Missing Lines Here ##########
...
def comput_gain_ratio(data, att_idx):
    ...
    i = compute_info_gain(data, att_idx)
    if i < 1e-9:
        return 0

    n = data.num_instances
    splits = data.split(att_idx)
    info = -np.sum([x.num_instances * np.log(x.num_instances / n) / n for x in splits.values() if x.num_instances])
    gain_ratio = i / info
    return gain_ratio
...
if __name__ == '__main__':
    if len(sys.argv) < 1 + 1:
        print('--usage python3 %s data [0/1, 0-Information Gain, 1-Gain Ratio, default: 0]' % sys.argv[0],
              file=sys.stderr)
        sys.exit(0)
    random.seed(27145)
    ...
    for train_keys, test_keys in kf.split(range(data.num_instances)):
        ...
        num_correct_predictions = sum(
            [1 if predictions[i] == test_data.label[i] else 0 for i in range(test_data.num_instances)])
